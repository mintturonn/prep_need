...
# parameters
par <- read_excel(here('data/params.xlsx'), sheet = 'nat_hist')

id_beta <- which(params$prior_shape=="beta")
id_lognrm <- which(params$prior_shape=="log-normal")
...
# read_excel(here("data/SVI2018_US_COUNTY.xlsx")) %>%
#   select(ST, STATE, ST_ABBR, COUNTY, FIPS, LOCATION, AREA_SQMI, E_TOTPOP, M_TOTPOP, RPL_THEME1, RPL_THEME2, RPL_THEME3, RPL_THEME4, RPL_THEMES)-> svi
...
# pop.m.all$msm_denom <- pop.m.all$msm
# pop.m.all$msw_denom <- pop.m.all$msw
# 
# pop.m.all$msm_denom[pop.m.all$Age.Group=="13-17"] <- pop.m.all$msm[pop.m.all$Age.Group=="13-24"]
# pop.m.all$msm_denom[pop.m.all$Age.Group=="18-24"] <- pop.m.all$msm[pop.m.all$Age.Group=="13-24"]
# 
# pop.m.all$msw_denom[pop.m.all$Age.Group=="13-17"] <- pop.m.all$msw[pop.m.all$Age.Group=="13-24"]
# pop.m.all$msw_denom[pop.m.all$Age.Group=="18-24"] <- pop.m.all$msw[pop.m.all$Age.Group=="13-24"]
# 
...
#  group_by(state.x, FIPS, agecat, `Transmission Category`) %>%
 ...
 # select(FIPS, state.x, agecat, prev, diag, msw) %>%
  ...
  left_join(hiv_vsuppr[hiv_vsuppr$Sex!="Female",], by = c("FIPS", "Age.Group")) -> pop.wsm
...
hiv_age_het_prev  %>%
 ...
 # group_by(state.x, FIPS, agecat, `Transmission Category`) %>%
  ...
  left_join(hiv_vsuppr[hiv_vsuppr$Sex!="Male",], by = c("FIPS", "Age.Group")) -> pop.msw
...
hiv_age_msm_prev  %>%
 ...
 # group_by(state.x, FIPS, agecat) %>% # attribute all to MSM contact (combines IDU and MSM)
  ...
  left_join(hiv_vsuppr[hiv_vsuppr$Sex=="Male",], by = c("FIPS", "Age.Group")) -> pop.msm

pop.msm$prev_min <- pop.msm$prev * 0.9
pop.msm$prev_max <- pop.msm$prev * 1.1

pop.msw$prev_min <- pop.msw$prev * 0.9
pop.msw$prev_max <- pop.msw$prev * 1.1

pop.wsm$prev_min <- pop.wsm$prev * 0.9
pop.wsm$prev_max <- pop.wsm$prev * 1.1
