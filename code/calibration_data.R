...
read_csv(here("incidence_data/AtlasPlusTableData_incid_state.csv"), skip=10) %>%
  ...
  filter(!is.na(rate)) %>%
  select(state, cases, cases_ll, cases_ul, population) -> df_incid_state0
...
read_csv(here("incidence_data/AtlasPlusTableData_incid_national.csv"), skip=10) %>%
  filter(Year=="2022") %>%
  mutate(cases = as.numeric(gsub(",", "", Cases)),
         cases_ll = as.numeric(gsub(",", "", `Cases LCI`)),
         cases_ul = as.numeric(gsub(",", "", `Cases UCI`))) %>%
  mutate(population = as.numeric(gsub(",", "", Population))) %>%
  select(Year, cases, cases_ll, cases_ul, population) %>%
...
read_csv(here("incidence_data/AtlasPlusTableData_incid_raceth.csv"), skip=10) %>%
  filter(Year=="2022") %>%
  mutate(cases = as.numeric(gsub(",", "", Cases))) %>%
  mutate(population = as.numeric(gsub(",", "", Population))) %>%
  mutate(rate = as.numeric(gsub(",", "", `Rate per 100000`))) %>%
  ...
  mutate(race = case_when(
    ...
    TRUE ~ `Race/Ethnicity` )) %>%
  select(race, cases, population) -> df_incid_raceth

# cdc hiv data national age
read_csv(here("incidence_data/AtlasPlusTableData_incid_age.csv"), skip=10) %>%
  filter(Year=="2022") %>%
  mutate(cases = as.numeric(gsub(",", "", Cases))) %>%
  mutate(population = as.numeric(gsub(",", "", Population))) %>%
  mutate(rate = as.numeric(gsub(",", "", `Rate per 100000`))) %>%
...
# cdc hiv data national age
read_csv(here("incidence_data/AtlasPlusTableData_incid_transmcat.csv"), skip=10) %>%
  filter(Year=="2022") %>%
  mutate(cases = as.numeric(gsub(",", "", Cases))) %>%
  mutate(population = as.numeric(gsub(",", "", Population))) %>%
  mutate(rate = as.numeric(gsub(",", "", `Rate per 100000`))) %>%
...
read_csv(here("incidence_data/AtlasPlusTableData_incid_transmcat_phet.csv"), skip=10) %>%
  filter(Year=="2022") %>%
  mutate(cases = as.numeric(gsub(",", "", Cases))) %>%
  mutate(population = as.numeric(gsub(",", "", Population))) %>%
  mutate(rate = as.numeric(gsub(",", "", `Rate per 100000`))) %>%
...
read_csv(here("incidence_data/AtlasPlusTableData_incid_msm_age.csv"), skip=10) %>%
  filter(Year=="2022") %>%
