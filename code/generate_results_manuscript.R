...
results %>%
  ...
  labs(
    x = "Average HIV incidence per 100 persons",
    y = "Cumulative percent of population size",
    color = "Simulation") +
  theme(
    #panel.grid.minor = element_blank(),
    axis.line = element_line(color = "black"),
...
ggplot( aes(x = 100*cumprop_prepind_trnsm, y = 100*cumprop_popinc_trnsm, identity=simulation)) +
  ...
  labs(
    x = "Cumulative percent of population size",
...
results2 %>%
  ...
  mutate(Allocation = recode(Allocation,
                            "sum_prep100" = "100% of HIV infections",
                            "sum_prep10" = "90% of HIV infections",
                            "sum_prep10.2" = "100% to top 90%, 5% to bottom 10%",
                            "sum_prep25" = "75% of HIV infections",
                            "sum_prep25.2" = "100% to top 75%, 5% to bottom 25%")) %>%
  ungroup() %>%
  bind_rows(tibble(
    trnsm = c("pwid", "wsm", "msw", "msm"),
    Allocation = c("Cost-effectiveness threshold approach"),
  ...
  mutate(Allocation = factor(Allocation, 
                             levels = c("Former CDC estimate",
                                        "Cost-effectiveness threshold approach",
                                        "100% of HIV infections",
                                        "90% of HIV infections",
                                        "100% to top 90%, 5% to bottom 10%",
                                        "75% of HIV infections",
...
df_trnsm1  %>%
  ...
  filter(Allocation != "100% to top 90%, 5% to bottom 10%" & Allocation != "100% to top 75%, 5% to bottom 25%") %>%
  mutate(trnsm = toupper(as.character(trnsm)), 
         trnsm = factor(trnsm, levels = c("MSM", "PWID", "WSM", "MSW"))) %>%
  ...
  # scale_y_log10( breaks = 10^(0:7), labels = scales::comma ) +
  scale_fill_manual(values = c("#8C510A", "#DFC27D", "purple","#BDD7E7", "#F46D43") ) -> p3 #"#D73027", 
...
results2 %>%
  ...
  ungroup() %>%
  mutate(mean_prep = ifelse(Allocation == "Cost-effectiveness threshold approach" | Allocation == "Former CDC estimate", mean_prep/pop, mean_prep )) %>%
  mutate(Allocation = factor(Allocation, 
                             levels = c("Former CDC estimate",
                                        "Cost-effectiveness threshold approach",
                                        "100% of HIV infections",
...
df_trnsm2  %>%
  ...
  scale_y_continuous(labels = comma) + ylim(c(0, 80)) +
  scale_fill_manual(values = c("#8C510A", "#DFC27D", "purple","#BDD7E7", "#F46D43") ) -> p3.2
...
df_race %>%
 # filter(Allocation != "100% of HIV infections") %>%
  filter(Allocation != "100% to top 90%, 5% to bottom 10%" & Allocation != "100% to top 75%, 5% to bottom 25%") %>%
  mutate(trnsm = toupper(as.character(trnsm)), 
         trnsm = factor(trnsm, levels = c("MSM", "PWID", "WSM", "MSW"))) %>%
  ...
  theme(
    ...
    plot.background = element_rect(fill = "white")) +
  scale_y_continuous(labels = comma) + 
  scale_fill_manual(values = c("purple", "#BDD7E7", "#F46D43" ) )  -> p4.2
...
df_race2 %>%
  ...
  scale_y_continuous(labels = comma) + #ylim(c(0, 80)) +
  scale_fill_manual(values = c("purple","#BDD7E7", "#F46D43" ) ) -> p4.1
...
cdc_prep_need %>% 
  ...
  mutate(lower_95 = NA,
         upper_95 = NA,
         Allocation = "Cost-effectiveness threshold approach")  %>%
  group_by(state, Allocation) %>%
  summarize(mean_prep = sum(mean_prep),
            lower_95 = sum(lower_95),
            upper_95 = sum(upper_95)) %>%
...
#  scale_x_continuous(labels = comma, limits = c(0, NA)) + 
  scale_color_manual(values = c("#8C510A", "#DFC27D","purple","#BDD7E7", "#F46D43" ) )  -> p5
...
df_state2 %>% 
  ...
  scale_color_manual(values = c("#8C510A", "#DFC27D","purple","#BDD7E7", "#F46D43" ) )  -> p5.2
...
df_age %>%
  ...
  facet_wrap(~trnsm, ncol=4, scales="free") + 
  scale_y_continuous(labels = comma) + 
  scale_fill_manual(values = c("purple","#BDD7E7", "#F46D43" )) -> p4
...
results2 %>%
  ...
  mutate(Allocation = recode(Allocation,
                             ...
                             "sum_prep25" = "75% of HIV infections",
                             "sum_prep25.2" = "100% to top 75%, 5% to bottom 25%")) %>%
  arrange(sex, Allocation) %>%
  select(`Sex` = sex, Allocation, `mean (95% UI)` = mean_CI) %>%
  pivot_wider(id_cols = "Sex", names_from = "Allocation", values_from = "mean (95% UI)") -> df_pwid_tab

write_csv(df_pwid_tab, "~/prep_need/output_data/pwid_tab.csv")
...
results2 %>%
  ...
  ungroup() %>%
  mutate(Allocation = factor(Allocation, 
                             levels = c("100% of HIV infections",
                                        "90% of HIV infections",
                                        "100% to top 90%, 5% to bottom 10%",
                                        "75% of HIV infections",
                                        "100% to top 75%, 5% to bottom 25%"))) %>%
  select(sex, Allocation, mean_prep, lower_95, upper_95, pop)  %>%
  mutate(
    mean_prep_fmt = format_sigfig2(100*mean_prep),
    lower_95_fmt = format_sigfig2(100*lower_95),
    upper_95_fmt = format_sigfig2(100*upper_95) ) %>%
  ...
  arrange(sex, Allocation) %>%
  select(`Sex` = sex, Allocation, `mean (95% UI)` = mean_CI) %>%
  pivot_wider(id_cols = "Sex", names_from = "Allocation", values_from = "mean (95% UI)") -> df_pwid_tab2

write_csv(df_pwid_tab2, "~/prep_need/output_data/pwid_rate_tab.csv")
