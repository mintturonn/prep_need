...
######################################################
sample.prior <- function (B0, pars_nat, pars_state, id_beta, id_lognr, id_gamma, id_normal, id_unif, id_fixed) {
  ...
  params.pwid1 <-   array(dim = c(B0, length(pars_state$state)), dimnames = list(NULL, paste0(pars_state$state, "_pr.pwid") ))
  params.pwid2 <-   array(dim = c(B0, length(pars_state$state)), dimnames = list(NULL, paste0(pars_state$state, "_vs.pwid") ))
  ...
  for (i in id_gamma) {
    params0[,i] <- rgamma(B0, as.numeric(pars_nat$shape1[i]), as.numeric(pars_nat$shape2[i]))
  }
  
  for (i in id_normal) {
    params0[,i] <- rnorm(B0, as.numeric(pars_nat$shape1[i]), as.numeric(pars_nat$shape2[i]))
  }
  ...
  for(s in 1:nrow(pars_state)){
    ...
    params.pwid1[,s] <- runif(B0, pars_state$pwid_prev_min[s], pars_state$pwid_prev_min[s])
    ...
    params.pwid2[,s] <- runif(B0, pars_state$pwid_vsupp_min[s],pars_state$pwid_vsupp_max[s])
